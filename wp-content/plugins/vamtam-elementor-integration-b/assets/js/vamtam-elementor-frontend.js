	...
	var VAMTAM_ELEMENTOR = {
		...
		VamtamActionLinksHandler: {
			...
			popupActionLinks: function() {
				...
				const handleUseVamtamPopupToggle = function() {
					const actionLinkedEls = $( 'a[href^="#elementor-action"]' );

					if ( ! actionLinkedEls.length ) {
						return;
					}

					$.each( actionLinkedEls, ( i, actionLinkedEl ) => {
						const actionLink = $( actionLinkedEl ).attr( 'href' ),
							settings = _self.utils.getSettingsFromActionLink( actionLink );

						if ( settings && settings.use_vamtam_popup_toggle ) {
							const $widgetEl = $( actionLinkedEl ).parents( '.elementor-widget' ).first();
							if ( $widgetEl.length ) {
								$widgetEl.addClass( 'vamtam-popup-toggle' );
							}
						}
					} );
				};
				handleUseVamtamPopupToggle()
			},
			utils: {
